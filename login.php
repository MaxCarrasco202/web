...
if ($query) {
    ...
    $message = "¡Gracias por registrarte en nuestro sitio! Por favor, haz clic en el siguiente enlace para confirmar tu cuenta: http://tu-sitio.com/confirmar.php?email=".urlencode($email);
    $headers = "From: tu-correo@dominio.com" . "\r\n" .
               "Reply-To: tu-correo@dominio.com" . "\r\n" .
               "X-Mailer: PHP/" . phpversion();

    if (mail($to, $subject, $message, $headers)) {
        echo "<script>alert('Estás registrado con éxito. Se ha enviado un correo de confirmación a tu dirección de correo electrónico. Por favor, verifica tu cuenta.');</script>";
    } else {
        echo "<script>alert('Estás registrado con éxito, pero no pudimos enviar el correo de confirmación. Por favor, ponte en contacto con el soporte.');</script>";
    }
} else {
...
<head>
	...
	<title>GameTech Abancay | Signi-in | Signup</title>
...
<body class="cnt-home">
    ...
    <input type="text" class="form-control unicase-form-control text-input" id="contactno" name="contactno" pattern="9\d{8}" required>
    <small>El número debe comenzar con 9 y tener un total de 9 dígitos.</small>
</div>
